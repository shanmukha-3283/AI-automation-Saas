...
import { leadQualifierGraph } from '../../agent/index.js';
...
webhookRoute.post('/:clientId', async (c) => {
    try {
        ...
        if (authToken && authToken !== 'your-twilio-auth-token' && process.env.NODE_ENV !== 'development') {
            ...
            const isValid = twilio.validateRequest(authToken, twilioSignature, url, rawBody);
            if (!isValid) {
        ...
        const message = WhatsAppAdapter.parseIncomingWebhook(body);
        ...
        try {
            ...
            console.warn('[RAG] Qdrant search failed, falling through to LLM:', e.message);
        }
        let finalResponse = '';
        ...
        if (faqResults.length > 0 && faqResults[0].score > 0.85) {
            ...
            const langGraphMessages = dbMessages.map(msg => msg.role === 'user' ? new HumanMessage(msg.content) : new AIMessage(msg.content));
            ...
            const resultState = (await leadQualifierGraph.invoke(initialState));
            ...
            if (resultState.qualificationStatus && resultState.qualificationStatus !== 'pending') {
                ...
                if (updatedLead)
                    io.to(client.id).emit('lead_updated', updatedLead);
        ...
        const adapter = new WhatsAppAdapter(client.twilioAccountSid || undefined, client.twilioAuthToken || undefined, client.twilioPhoneNumber || undefined);

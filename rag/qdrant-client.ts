 ...
 */
export async function ingestFAQ(question: string, answer: string) {
  try {
 ...
 */
export async function searchFAQ(query: string, limit: number = 3) {
  try {
    ...
    return searchResults.points.map((result: any) => ({
      score: result.score,
      question: result.payload?.question as string,
      answer: result.payload?.answer as string,
    }));
  } catch (error) {
